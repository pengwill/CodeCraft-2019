...
//        return false;
    ...
    bool moveToNextRoad(Car & curCar, Cross & curCross);
    void forceCheck();
};
struct Cross {
    int id;
...
bool Road::moveToNextRoad(Car & curCar, Cross & curCross) {
    ...
    if(curCar.curPathId + 1 == curCar.path.size()) {
        ...
        if(moveDistance <= 0) {
            ...
            cout << "[M2nxtR] carID " << car_hashBack[curCar.id] << ", Road " << road_hashBack[id]
                 << " move to Pos " << carPosition <<  " cause nextRoad < 0" << endl;
#endif
                ...
                if(moveDistance >= nextRoad.length - lastCar.curPosition + 1) {
                    if(lastCar.status == waitingStatus) {
                        ...
                        if(lastCar.curPosition == nextRoad.length)  {
                            if(curCar.id == car_map[87312]) {
                                cout << "frontCar " << car_hashBack[lastCar.id] << " ";
                            }
                            continue;
                        }
                        else {
...
void Road::arrangeOnRoad(bool isPriorityOnly, const int & index) {
    for(auto curCarID = carInitList[index].begin(); curCarID != carInitList[index].end(); ) {
        if(isPriorityOnly) {
            ...
            if(cars[*curCarID].isPriority && cars[*curCarID].RealStartTime > systemTime) { ++curCarID; continue;}
            if(!cars[*curCarID].isPriority && cars[*curCarID].RealStartTime > systemTime) return;
        }
        int st = 0, ed = 0;
        if(isDuplex)
    ...
    for(int i = 0; i < 4; ++i) {
        if(order[i] == -1) continue;
            ...
            while(!curRoad.turing[index].empty()) {
                ...
                if(curRoad.moveToNextRoad(curCar, *this)) {
...
double round(double value, int bits) {
    stringstream ss;
    ss << fixed << setprecision(bits) << value;
    ss >> value;
    return value;
}
...
int main(int argc, char *argv[]) {
    ...
    for(auto & curRoad : roads) {
        if(!curRoad.carInitList[0].empty()) sort(curRoad.carInitList[0].begin(), curRoad.carInitList[0].end(), cmp_carInitList);
        if(!curRoad.carInitList[1].empty()) sort(curRoad.carInitList[1].begin(), curRoad.carInitList[1].end(), cmp_carInitList);
    }
...
//            cout << "[ROUND" << cnt++ << "] OnRoadCar " << onRoadCarsCounter << ",Waiting " << waitingCarsCounter << endl;
    ...
    if (isAllCarsReach) {
        ...
        cout << TE << " " << (int)TESum << " " <<   systemTime << " " << (time2 - time1) / 1000.0 << endl;
    }
    return 0;
}
